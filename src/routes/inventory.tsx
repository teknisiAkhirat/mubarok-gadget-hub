...
function InventoryPage() {
  ...
  async function save() {
    ...
    try {
      ...
      if (editId) {
        ...
        const { error } = await supabase.from("products").insert([{ ...payload, seller_id: "seller-mubarok" }]);
        if (error) throw error;
  ...
  async function markSold(id: string) {
    try {
      const { error } = await supabase.from("products").update({ stock: 0, sale_status: "terjual", is_active: false }).eq("id", id);
      if (error) throw error;
    ...
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Barang masuk, stok tersedia, dan status penjualan.</p>
        </div>
          ...
          <CardContent className="p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1">
                ...
                <Input value={form.merk} onChange={(e) => setForm({ ...form, merk: e.target.value })} placeholder="Samsung" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-muted-foreground">Tipe / Model</label>
                <Input value={form.tipe} onChange={(e) => setForm({ ...form, tipe: e.target.value })} placeholder="Galaxy M52" />
              </div>
              <div className="space-y-1 md:col-span-2">
                <label className="text-xs font-semibold text-muted-foreground">IMEI / Serial Number</label>
                <Input value={form.imei_or_sn} onChange={(e) => setForm({ ...form, imei_or_sn: e.target.value })} placeholder="IMEI atau SN" />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
              ...
              <div className="space-y-1">
                ...
                <select value={form.condition} onChange={(e) => setForm({ ...form, condition: e.target.value })} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
              ...
              <div className="space-y-1">
                ...
                <Input value={form.condition_note} onChange={(e) => setForm({ ...form, condition_note: e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-muted-foreground">Harga Modal (IDR)</label>
                <Input type="number" value={form.cost_price} onChange={(e) => setForm({ ...form, cost_price: +e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-muted-foreground">Harga Jual (IDR)</label>
              ...
              <div className="space-y-1 md:col-span-2">
                ...
                <textarea className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" rows={3} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
              </div>
            </div>
            <div className="mt-4 flex gap-2">
              ...
              <Button variant="outline" onClick={() => setShowForm(false)} disabled={busy}>Batal</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-muted-foreground">Item yang masih bisa dijual atau ditukar-tambah.</p>
      </div>
            ...
            <CardContent className="flex flex-1 flex-col gap-2 p-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{item.merk} · {item.tipe}</p>
                  <p className="text-xs text-muted-foreground">IMEI/SN: {item.imei_or_sn ?? "—"}</p>
                </div>
                <Badge variant="outline" className="capitalize">{item.condition_label}</Badge>
              </div>
              <div className="text-sm">
                <p className="text-xs text-muted-foreground">Modal</p>
                <p className="font-semibold">{(item.cost_price ?? 0) > 0 ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(item.cost_price as number) : "—"}</p>
              </div>
              ...
              <div className="mt-2 flex gap-2">
                <Button size="sm" variant="outline" onClick={() => startEdit(item)}><Pencil className="h-3.5 w-3.5" /></Button>
                <Button size="sm" variant="outline" onClick={() => markSold(item.id)} className="border-emerald-600 text-emerald-700 hover:bg-emerald-50">Terjual</Button>
                <Button size="sm" variant="outline" onClick={() => remove(item.id)} className="border-red-600 text-red-700 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
          ...
          {sold.map((item) => (
            <Card key={item.id} className="opacity-80">
              <CardContent className="p-4">
                <p className="font-semibold">{item.name}</p>
                <p className="text-xs text-muted-foreground">{item.merk} · {item.tipe}</p>
                <p className="text-sm font-bold">{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(item.price)}</p>
